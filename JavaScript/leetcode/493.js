...
var reversePairss = function (nums) {
  ...
  while (len > index) {
    ...
    if (!map.hasOwnProperty(tmp)) {
      ...
      res += Object.values(map[tmp]).reduce((t, e, n) => {
        return t + e * (n + 1)
      }, 0)
    }
    index++
...
const reversePairsRecursive = (nums, left, right) => {
  ...
  const mid = Math.floor((left + right) / 2) // 中数
  let ret = reversePairsRecursive(nums, left, mid) + reversePairsRecursive(nums, mid + 1, right) // 获取子目结果

  let i = left // left初始指针
  let j = mid + 1 // right初始指针
  /**
   * 该循环 是为了符合该方法的需求，获得反转数量
   * 子项中 [left(i), mid], [mid+1(j), right]
   * 两相对比, nums[i] > 2 * nums[j]
   * 获得反转的数量
   */
  while (i <= mid) {
 ...
 * 方法二：树状数组
 * 思路及解法
 * 树状数组支持的基本查询为求出 [1,val] 之间的整数数量。因此，对于 nums[i] 而言，我们首先查询 [1,2⋅nums[i]] 的数量，再求出 [1,maxValue] 的数量（其中 maxValue 为数组中最大元素的二倍）。二者相减，就能够得到以 i 为右端点的翻转对数量。
 * 由于数组中整数的范围可能很大，故在使用树状数组解法之前，需要利用哈希表将所有可能出现的整数，映射到连续的整数区间内。
 */
var reversePairs2 = function (nums) {
  const allNumbers = Array.from(new Set([...nums, ...nums.map(x => 2 * x)].sort((x, y) => x - y))) // 利用哈希表进行优化
